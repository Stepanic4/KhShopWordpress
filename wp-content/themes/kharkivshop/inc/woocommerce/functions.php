				...
				if ( is_shop() || is_product_tag() || is_product_category() ) :
					...
					if ( is_active_sidebar( 'KharkivShop-sidebar-shop-archive' ) ) :
						?>
							...
							<span class="KharkivShop-sidebar-open">
								<span class="icon-filter"></span>
							</span>
							<div class="col-sm-9 shop-with-sidebar" id="KharkivShop-blog-container">
...
if ( ! function_exists( 'shop_isle_shop_page_wrapper_end' ) ) {
	...
	function shop_isle_shop_page_wrapper_end() {
		?>
			...
			<?php if ( ( is_shop() || is_product_category() || is_product_tag() ) && is_active_sidebar( 'KharkivShop-sidebar-shop-archive' ) ) : ?>

				</div>

				<!-- Sidebar column start -->
				<div class="col-sm-3 col-md-3 sidebar sidebar-shop">
					<span class="KharkivShop-sidebar-close">
...
function shop_isle_woocommerce_scripts() {
	...
	wp_enqueue_style( 'KharkivShop-woocommerce-style1', get_template_directory_uri() . '/inc/woocommerce/css/woocommerce.css', array(), '11' );
}
...
function shop_isle_products_slider_on_single_page() {
	...
	if ( ( isset( $shop_isle_products_slider_single_hide ) && $shop_isle_products_slider_single_hide != 1 ) || is_customize_preview() ) :
			...
			echo '<div class="container">';
				...
				$shop_isle_products_slider_title    = get_theme_mod( 'shop_isle_products_slider_title', __( 'Exclusive products', 'KharkivShop' ) );
				$shop_isle_products_slider_subtitle = get_theme_mod( 'shop_isle_products_slider_subtitle', __( 'Special category of products', 'KharkivShop' ) );
...
function shop_isle_outofstock_notify_on_archives() {
	...
	if ( function_exists( 'method_exists' ) && method_exists( $product, 'managing_stock' ) && method_exists( $product, 'is_in_stock' ) ) {
		if ( ! $product->managing_stock() && ! $product->is_in_stock() ) {
			echo '<span class="onsale out-of-stock">' . esc_html__( 'Out of Stock', 'KharkivShop' ) . '</span>';
		}
	}
}
