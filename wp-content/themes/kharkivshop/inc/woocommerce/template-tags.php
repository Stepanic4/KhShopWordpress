...
if ( ! function_exists( 'shop_isle_cart_link' ) ) {
	...
	function shop_isle_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'KharkivShop' ); ?>">
			<?php
			...
			<span class="count">
				...
				echo wp_kses_data(
					...
					sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'KharkivShop' ), WC()->cart->get_cart_contents_count() )
				);
				?>
			</span>
		</a>
