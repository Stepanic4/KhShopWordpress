...
function shop_isle_products_slider_controls_customize_register( $wp_customize ) {
	...
	if ( ! class_exists( 'WooCommerce' ) ) {
		$shop_isle_require_woo = '<div class="KharkivShop-require-woo"><p>' . sprintf(
			/* translators: 1: Link to WooCommerce Plugin */
				__( 'To use this section, you are required to first install the  %1$s plugin', 'KharkivShop' ),
			sprintf(
				...
				esc_html__( 'WooCommerce', 'KharkivShop' )
			)
		) . '</p></div>';
	}
	...
	$wp_customize->add_section(
		...
		array(
			'title'       => __( 'Products slider section', 'KharkivShop' ),
	...
	$wp_customize->add_control(
		...
		array(
			'type'     => 'checkbox',
			'label'    => __( 'Hide products slider section on frontpage?', 'KharkivShop' ),
	...
	$wp_customize->add_control(
		...
		array(
			'type'     => 'checkbox',
			'label'    => __( 'Hide products slider section on single product page?', 'KharkivShop' ),
			'section'  => 'shop_isle_products_slider_section',
			'priority' => 2,
		)
	);
	...
	$wp_customize->add_setting(
		...
		array(
			...
			'sanitize_callback' => 'shop_isle_sanitize_text',
			'default'           => __( 'Exclusive products', 'KharkivShop' ),
		)
	);
	...
	$wp_customize->add_setting(
		...
		array(
			'transport'         => $selective_refresh,
			'sanitize_callback' => 'shop_isle_sanitize_text',
			'default'           => __( 'Special category of products', 'KharkivShop' ),
		)
	);
	...
	$wp_customize->add_control(
		'shop_isle_products_slider_subtitle',
		array(
			'label'    => __( 'Section subtitle', 'KharkivShop' ),
			'section'  => 'shop_isle_products_slider_section',
			'priority' => 4,
		)
	);
	...
	$shop_isle_prod_categories_array = array(
		'-' => __( 'Select category', 'KharkivShop' ),
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'        => 'select',
			'label'       => __( 'Products category', 'KharkivShop' ),
			'section'     => 'shop_isle_products_slider_section',
			'choices'     => $shop_isle_prod_categories_array,
			'priority'    => 5,
			'description' => __( 'If no category is selected , WooCommerce products from the first category found are displaying.', 'KharkivShop' ),
		)
	);
